...
Version 2 - instead of masking clouds, this script looks for cloud pixels within the lake area and excludes any images with clouds or cloud shadows
"""
import sys
import os
                ...
                for file in os.listdir(newDir):
                    ...
                    if file.endswith('ST_B10.TIF') or file.endswith('ST_B6.TIF'):
                        ...
                        if stats['sum'] is None:
                            ...
                            #get cloud values by unpacking the QA Pixel band
                            QA_unpacked = landsatQAmask.get_mask(QA_clip_array)
                            
                            #check for any zeros in the clipped array as these are clouds or shadows
                            if not np.all(QA_unpacked): #need to establish a threshold here
                                surf_temp.close()
                                QA_pixel.close()
                                continue
                            else:
                                #set zeros as no data, need to convert to float array
                                ST_float = ST_clip_array.astype('float')
